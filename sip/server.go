...
func (s *Server) handleMsg(c conn, m *message, at *util.Map[string, *activeTx], pt *util.Map[string, *passiveTx]) error {
	...
	if m.isReq {
		// 日志
		log.DebugfTrace(SIP, "read request from %s %s:%s\n%v", c.Network(), c.RemoteIP(), c.RemotePort(), m)
		// 事务，返回一定不为 nil
	...
	// 日志
	log.DebugfTrace(SIP, "read response from %s %s:%s\n%v", c.Network(), c.RemoteIP(), c.RemotePort(), m)
	// 响应消息
...
// Request 发送请求并等待响应
func (s *Server) Request(r *Request, a net.Addr, d any) error {
	// tcp
	if _a, ok := a.(*net.TCPAddr); ok {
		...
		// 请求
		return s.doRequest(c, r.message, d, &s.tcp.at)
	}
	// udp
	if _a, ok := a.(*net.UDPAddr); ok {
		...
		c.initAddr(_a)
		//
		return s.doRequest(c, r.message, d, &s.udp.at)
	}
	//
...
func (s *Server) doRequest(c conn, m *message, d any, at *util.Map[string, *activeTx]) error {
	// 事务
	...
	// 日志
	log.DebugfTrace(t.TxKey(), "write request to %s %s:%s\n%s", c.Network(), c.RemoteIP(), c.RemotePort(), t.writeData.String())
	// 立即发送
