...
func (m *URI) Dec(line string) bool {
	...
	if line != "*" {
		// scheme:
		m.Scheme, line = Split(line, ':')
		if m.Scheme == "" {
			return false
		}
		// name@domain
		m.Name, m.Domain = Split(line, '@')
	}
	//
	return true
}
...
func (m *Via) Dec(line string) bool {
	...
	// version
	s := strings.TrimPrefix(prefix, SIPVersion)
	if s == line {
		return false
	}
	// proto ，去掉第一个 '/' 即可
...
func (m *Via) Enc(w Writer) (err error) {
	...
	_, err = fmt.Fprintf(w, "Via: %s/%s %s", SIPVersion, m.Proto, m.Address)
	if err != nil {
		return err
	}
	// rport
...
func (m *Request) Response(status, phrase string) error {
	if phrase == "" {
	...
	m.Enc(&m.writeData)
	// 日志
	log.DebugfTrace(m.TxKey(), "write response to %s %s:%s\n%s", m.Network(), m.RemoteIP(), m.RemotePort(), m.writeData.String())
	// 立刻发送
	return m.write(m.writeData.Bytes())
}
