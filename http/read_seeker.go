...
const (
	readSeekCloserHeaderLen = 1024
)

var (
	errReadSeekCloserSeekOffset = errors.New("error seek offset")
)

// ReadSeekCloser 实现 io.ReadSeekCloseer，服务必须支持 Range 哦
...
func (r *ReadSeekCloser) Seek(offset int64, whence int) (int64, error) {
	...
	// 负数
	if offset < 0 {
		return r.offset, errReadSeekCloserSeekOffset
	}
	// 结尾
	if offset >= r.total {
		r.offset = offset
...
func (r *ReadSeekCloser) seek(offset int64) error {
	// 关闭上一个响应的 body
	res := r.res
	r.res = nil
	if res != nil {
...
// NewReadSeeker 返回新的 ReadSeekCloser ，开始会请求一次
func NewReadSeeker(url string, offset int64) (*ReadSeekCloser, error) {
	// 请求
